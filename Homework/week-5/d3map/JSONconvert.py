...
def make_json(url):
    ...
    # Lege dict om de data in te stoppen
    pointsdict = {}
    for countryinfo in countrylist:
    	# Lege dict voor landdata
        infodict = {}

        # Neem de naam van het land
        infodict['name'] = DOM(countryinfo.content).by_tag("a")[0].attrs.get("title","").encode("utf-8")
        # Neem het percentage renewable energy van het land
        infodict['value'] = float(DOM(countryinfo.content).by_tag("td")[8].content.encode("utf-8").replace(",", "."))

        # Bepaal in welke categorie het land hoort
        if infodict['value'] < 20:
            infodict['fillKey'] = "LOW"
        elif infodict['value'] < 40:
            infodict['fillKey'] = "BELAVG"
        elif infodict['value'] < 60:
            infodict['fillKey'] = "AVG"
        elif infodict['value'] < 80:
            infodict['fillKey'] = "ABVAVG"
        else:
            infodict['fillKey'] = "HIGH"

        # Zoek de drieletterige code van het land op
        code = ""
    ...
    # Dump de dictionary als JSON naar de textfile data.json
    json.dump(pointsdict, open('data.json', 'wb'))
